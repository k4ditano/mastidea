...
export default function IdeaPage() {
  ...
  const [replyingToExpansion, setReplyingToExpansion] = useState<string | null>(
    null
  );
  ...
  const handleReplyToExpansion = async (
    expansionId: string,
    message: string
  ) => {
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white relative">
      ...
      <div className="max-w-3xl mx-auto px-4 py-6 sm:px-6">
        ...
        <div className="mb-4">
          <IdeaActions
            idea={idea}
            onUpdate={(updatedIdea) => setIdea(updatedIdea)}
            isOwner={isOwner}
          />
        </div>
        ...
        <div className="space-y-4 pb-64">
          ...
          <div className="flex justify-start">
            <div className="max-w-[85%] bg-gradient-to-br from-gray-100 to-gray-50 text-gray-800 rounded-2xl rounded-tl-sm px-6 py-4 shadow-md border-2 border-gray-200">
              ...
              <p className="text-sm whitespace-pre-wrap leading-relaxed text-gray-700 mb-3">
                {idea.content}
              </p>
              ...
              {idea.tags && idea.tags.length > 0 && (
                <div className="flex flex-wrap gap-2 pt-3 border-t border-gray-300">
                  {idea.tags.map((ideaTag) => (
                    <Link
                      key={ideaTag.tagId}
                      href={`/ideas?tag=${encodeURIComponent(
                        ideaTag.tag.name
                      )}`}
                  ...
                  <div className="max-w-[85%] bg-white rounded-2xl rounded-tl-sm px-6 py-4 shadow-md border border-gray-200">
                    <div className="flex items-center space-x-2 mb-3">
                      <span className="text-xl">
                        {EXPANSION_TYPE_LABELS[expansion.type].emoji}
                      </span>
